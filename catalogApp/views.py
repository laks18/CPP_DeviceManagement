...
def addProductCategory(request, pid=None):
    ...
    if request.method == 'POST':
        ...
        if pid:
            ...
            catFile= request.FILES['image'].name;
            print(catFile);
            upload_file("catalogimage01", os.path.join("media",catFile)) #adding place to store image
            messages.success(request, "Category Added")
        return redirect('vwproductCategory')
    return render(request, "add_product_category.html", locals())


#adding S3 code below
...
def ordernow(request, pid):
    ...
    if request.method == 'POST':
        #below is the SNS service used code
        orderid=1000
        if dataorder:
        ...
        send.send_SMS_message("+353894269781" ,"Thank you ! Your Request has been submitted")
